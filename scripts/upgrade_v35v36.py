...
V35 = V34 重跑零误差（33.36%/-22.66%/Calmar 1.472/累计 599.39x，无杠杆基础版；含202608后32.76%）
V36 = V33 重跑零误差（42.43%/MinMax 校验 41.47%/-33.65%/Calmar 0.82，1.5x 杠杆版）
...
def nav_series(rows, key="adj_ret", start="201204", end="202608"):
    months, nav = [], []
    cur = 1.0
    for r in rows:
        ym = r["ym"].strip()
...
assert abs(met35["annual_return"] - 32.76) < 0.01, f"V35 年化校验失败: {met35['annual_return']}"
assert abs(met35["mdd"] - (-22.66)) < 0.05, f"V35 MDD 校验失败: {met35['mdd']}"
assert abs(met36["annual_return"] - 41.47) < 0.01, f"V36 年化校验失败: {met36['annual_return']}"
...
data = {
    "meta": {
        ...
        "note": "数据为14年历史回测（2012.04-2026.08，173个月），含成本口径（佣金万2.5+印花税0.05%）。V35=基础版（V31+帕伯莱红旗审查，无杠杆）；V36=杠杆版（V32+红旗审查，1.5x融资，内部研究用）。红旗审查有效性主要在2016-2026数据完整期验证。历史回测不代表未来收益。",
...
stock_data = {
    ...
    "window_126": {
        ...
        "annual": 27.91, "mdd": -16.78, "calmar": 1.663, "cum": 13.25,
    },
...
vs = {
    "version": "Standard",
    "window": {"start": "201603", "end": "202608", "months": 126},
    ...
    "strategies": {
        ...
        "stock": {"label": "股票投资策略(V35)", "annual": 27.91, "mdd": -16.78, "calmar": 1.663, "sharpe": 0.98, "cum": 13.25, "vol": 25.32},
    },
