import"./charts.CFgpAu5l.js";var e=JSON.parse(document.getElementById(`__V35_PICKS__`)?.textContent??`{"months":[]}`),t=JSON.parse(document.getElementById(`__HISTORY_DATA__`)?.textContent??`{"months":[]}`),n=JSON.parse(document.getElementById(`__STRATEGY_META__`)?.textContent??`{}`),r=document.getElementById(`rec-search`),i=document.getElementById(`rec-results`),a=`v35`,o=document.getElementById(`tab-v35`),s=document.getElementById(`tab-v31`);o?.addEventListener(`click`,()=>{a=`v35`,c(),u(r?.value??``)}),s?.addEventListener(`click`,()=>{a=`v31`,c(),u(r?.value??``)});function c(){let e=e=>{e&&(e.style.borderColor=`var(--gold,#d9a441)`,e.style.background=`rgba(217,164,65,0.12)`,e.style.color=`var(--gold,#b8860b)`,e.style.fontWeight=`700`)},t=e=>{e&&(e.style.borderColor=`var(--border-strong)`,e.style.background=`transparent`,e.style.color=`var(--text-3)`,e.style.fontWeight=`600`)};a===`v35`?(e(o),t(s)):(e(s),t(o))}function l(e){let t=e.replace(/[^0-9]/g,``);return t.length===6?t:t.length===7?t.slice(0,4)+t.slice(5):t}function u(r){let o=l(r??``);if(!o||o.length!==6){i.innerHTML=`<p style="color:var(--text-3);font-size:14px;">请输入 6 位月份，如 202607</p>`;return}let s=a===`v35`,c=s?e.months?.find(e=>e.ym===o):t.months?.find(e=>e.ym===o),u=n.months?.indexOf(o),d=u>=0?s?n.monthly_compare?.v35?.[u]:n.monthly_compare?.v31?.[u]:null,f=u>=0?n.monthly_compare?.hs300?.[u]:null;if(!c&&d==null){i.innerHTML=`<p style="color:var(--red);font-size:14px;">未找到 ${o} 的数据</p>`;return}let p=``;if(d!=null&&(p+=`
          <div style="background:var(--surface-alt);border-radius:10px;padding:16px;margin-bottom:14px;">
            <b style="font-size:15px;color:var(--navy-800);">${o.slice(0,4)} 年 ${o.slice(4)} 月 · 策略表现（${s?`V35 基础版`:`V31 沿革`}）</b>
          ...
          <div style="background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:16px;margin-bottom:14px;">
            <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:4px;">
              <b style="font-size:15px;color:var(--navy-800);">${o.slice(0,4)} 年 ${o.slice(4)} 月 · 选股名单（${t.length} 只）${s?` · V35`:` · V31`}</b>
              ${e.join(``)}
            </div>
            ...
            <p style="font-size:12px;color:var(--text-3);margin-top:8px;">
              ${s?`V35 档案：当月收益 = 个股在持仓月实际收益（%）。红旗删除 = 帕伯莱审查命中被剔除的股票。投前/投后明细见回测系统原始 CSV（v35_picks.json 仅含精简字段）。`:`权重 = FCF/EV 排名加权（rank+0.5 归一化），非等权。投前月 = 选股时点上一月收益（止损过滤依据）；投后月 = 持仓月实际收益。`}
            </p>
