...
rm -rf index.html _astro about archive backtest favicon.ico favicon.png favicon.svg macro methodology philosophy portfolio privacy ranking services strategies tools .nojekyll
cp -r "$TMP"/* . && touch .nojekyll
# ⚠️ 显式文件列表，禁止 git add -A（gh-pages 分支无 .gitignore，-A 会把 node_modules 提交进仓库并污染分支切换）
git add index.html _astro about archive backtest favicon.ico favicon.png favicon.svg macro methodology philosophy portfolio privacy ranking services strategies tools .nojekyll
